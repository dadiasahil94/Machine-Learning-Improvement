...
def GENERATE(mean_vector , cov_matrix, no_pts=100,class_label=1):
    ...
    '''
    data = np.random.multivariate_normal(class_label ,mean_vector , cov_matrix , no_pts)
    return data

def GENERATE_RANDOM(no_class=2 , no_features = 2 ,no_pts=100):
...
def DISCRIMINANT_FUNCTION(class_parameters):
    ...
    for i in xrange(no_class):
        ...
        current_class_prior = class_parameters[i][3]
        ...
        ABC_list.append((current_class_prior,A,B,C))    #(classlabel ,A, B ,C)
...
def main():
    ...
    data , no_class , no_features , class_parameters = GENERATE_RANDOM(no_class=3 , no_features =3, no_pts=250)
    hf_LDA.PLOT_3D( data[ : , 0] ,data[ : , 1] , data[ : , 2] )
    # print data , class_parameters
    DISCRIMINANT_FUNCTION(class_parameters)
